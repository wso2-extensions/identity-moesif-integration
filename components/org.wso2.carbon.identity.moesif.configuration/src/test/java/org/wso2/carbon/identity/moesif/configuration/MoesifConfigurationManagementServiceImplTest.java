...
public class MoesifConfigurationManagementServiceImplTest {
    ...
    public void testAddPublisherUnsupportedNameThrows() throws Exception {
        ...
        service.addMoesifPublisher("wrongName", "apiKey", "orgId", "appId");
    }
    ...
    @Test(expectedExceptions = MoesifConfigurationManagementClientException.class)
    public void testGetDashboardTokenPublisherNotFoundThrows() throws Exception {

        stubResourceNotFound(PUBLISHER_NAME);
        service.getDashboardViewerToken(PUBLISHER_NAME);
    }

    /**
     * When the publisher exists but has no {@code moesifOrgId} attribute, the method must throw
     * a client exception rather than proceeding to the Moesif portal API call.
     */
    @Test(expectedExceptions = MoesifConfigurationManagementClientException.class)
    public void testGetDashboardTokenMissingOrgIdThrows() throws Exception {

        Resource resource = buildResource(PUBLISHER_NAME); // no moesifOrgId attribute
        when(mockConfigManager.getResource(eq(RESOURCE_TYPE), eq(PUBLISHER_NAME))).thenReturn(resource);
    ...
    /**
     * Build a {@link Resource} that includes the given {@link Attribute} entries.
     */
    private static Resource buildResourceWithAttributes(String name, Attribute... attributes) {

        Resource resource = buildResource(name);
        List<Attribute> attrList = new ArrayList<>();
        for (Attribute attr : attributes) {
            attrList.add(attr);
        }
        resource.setAttributes(attrList);
        return resource;
    }
}
